...
for nombre_telefono in nombres_telefonos:
    ...
    df = pd.read_csv(f'C:/Users/jaume/Documents/VISUAL CODES/TFG/muestrasTwoSided/muestras2TwoSided_{nombre_telefono}.csv')
...
print(f'\nMedia Porcentaje Muestras Válidas por Ángulo para cada Teléfono y MAC')
print(df_muestras_validas_media_pivot)
#guardamos el dataframe en un archivo csv
df_muestras_validas_media_pivot.to_csv('C:/Users/jaume/Documents/VISUAL CODES/TFG/Tablas2Side/muestras_validas_media_pivot.csv')
...
print(df_BSlogrado)
#guardamos el dataframe en un archivo csv
df_BSlogrado.to_csv('C:/Users/jaume/Documents/VISUAL CODES/TFG/Tablas2Side/BSlogrado.csv')

#####------------------------ANALISIS DEL RMSE 4 GRAFICAS------------------------#####
...
df_rmse.to_csv('C:/Users/jaume/Documents/VISUAL CODES/TFG/Tablas2Side/RMSE.csv')
# Graficar los resultados del RMSE para cada teléfono
for nombre_telefono in nombres_telefonos:
    plt.figure(figsize=(12, 8))
    for band_name in mac_to_band.values():
        if band_name in rmse_resultados[nombre_telefono]:
            rmse_data = rmse_resultados[nombre_telefono][band_name]
            plt.plot(list(rmse_data.keys()), list(rmse_data.values()), marker='o', label=band_name)
...
# Guardar el DataFrame en un archivo CSV
df_ranging_error.to_csv('C:/Users/jaume/Documents/VISUAL CODES/TFG/Tablas2Side/RangingError.csv')
...
for nombre_telefono in nombres_telefonos:
    ...
    plt.legend(title='Bandas')
    plt.show()
#####------------------------ANALISIS DE PDF RANGING ERROR SOLO PARA L2.4 ------------------------#####
# Colores específicos para cada banda
colores_bandas = {
    'L5DFS': '#1f77b4',      # Azul
    'L5indoor': '#ff7f0e',   # Naranja
...
# Crear gráficos de densidad solo para la banda L2.4 para cada teléfono
for nombre_telefono in nombres_telefonos:
    plt.figure(figsize=(14, 10))
    # Iterar sobre cada banda
    for band_name, color in colores_bandas.items():
        # Incluir solo la banda 'L2.4'
        if band_name in ranging_error_resultados[nombre_telefono] and band_name == 'L2.4':
            # Extraer los valores de Ranging Error
            ranging_data = ranging_error_resultados[nombre_telefono][band_name].values()
            # Convertir a una lista y eliminar NaN si existen
            ranging_data = [val for val in ranging_data if not np.isnan(val)]
            # Si hay suficientes datos, graficar la densidad
            if len(ranging_data) > 0:
                sns.kdeplot(ranging_data, label=band_name, fill=True, bw_adjust=0.3, color=color)
    
    # Añadir detalles a la gráfica
    plt.title(f'Densidad del Ranging Error - {nombre_telefono} (Solo L2.4)')
...
# Guardar el DataFrame en un archivo CSV
df_desviacion_estandar.to_csv('C:/Users/jaume/Documents/VISUAL CODES/TFG/Tablas2Side/DesviacionEstandar.csv')
...
print(df_offset_5m)
# Guardar el DataFrame en un archivo CSV
df_offset_5m.to_csv('C:/Users/jaume/Documents/VISUAL CODES/TFG/Tablas2Side/Offset_5m.csv')
